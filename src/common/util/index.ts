import {SkyeKiwi, SkyeKiwiBrowser} from '../../index'
...
const serialize = (object: any) : string => {
  for (let key in object) {
    ...
    try {
      if (object[key].constructor === SkyeKiwi.Chunks ||
      ...
      try {
        if (object[key].constructor === SkyeKiwiBrowser.Chunks ||
          object[key].constructor === SkyeKiwiBrowser.Seal ||
          object[key].constructor === SkyeKiwiBrowser.File ||
          object[key].constructor === SkyeKiwiBrowser.IPFS ||
          object[key].constructor === SkyeKiwiBrowser.EncryptionSchema ||
          object[key].constructor === SkyeKiwiBrowser.Metadata) {
          object[key] = object[key].serialize()
        }
        else if (Array.isArray(object[key])) {
          object[key] = serialize(object[key])
        ...
        console.error(err)
        throw new Error("serializing err - Util.serialize")
      }
    }
  }
